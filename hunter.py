...
# --- SETTINGS (NAME HUNTER EDITION) ---
# Alphabet strictly from "qadrisaifnaimuddin"
CHARSET = "qadrisfnmu" 
SAFE_DELAY = 15 
BATCH_SIZE = 100 
    ...
    if DISCORD_WEBHOOK:
        try:
            requests.post(DISCORD_WEBHOOK, json={
                "content": f"🚩 **[NAME HUNT HIT]:** `{username}` is available! Grab it: https://www.instagram.com/accounts/emailsignup/"
            })
        except Exception as e:
            print(f"Failed to send Discord notification: {e}")
    else:
        print(f"!!! NAME HUNT HIT: {username} !!! (No Discord Link)")
...
def check_availability(username):
    ...
    try:
        ...
        # Step 2: Strict Signup API check
        ...
        data = {
            "email": f"name_hunt_{int(time.time())}@gmail.com",
            "username": username,
            "first_name": "Hunt",
...
def run_batch():
    ...
    print(f"Starting Name Hunt (Alphabet: {CHARSET}) | Length {length}, Index {start_idx}")
    ...
    for combo in iterator:
        ...
        # All combinations are valid since no periods/underscores in alphabet
